...
def _year_pillar(year: int, month: int, day: int) -> tuple[str, str]:
    ...
    The Chinese astrological year begins at Lìchūn (立春, Start of Spring),
    approximately 4 February each year.  Births before that date belong to
    the *previous* astrological year.
    """
    astro_year = year if (month > 2 or (month == 2 and day >= 4)) else year - 1
