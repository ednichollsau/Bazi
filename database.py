...
def init_db():
    ...
    try:
        ...
        """)
        # Add zones column to patient_notes if it doesn't exist yet (migration)
        _run(conn, """
...
def create_patient_note(patient_id: int, note_date: str, content: str, zones: list = None) -> dict | None:
    if not DATABASE_URL:
        return None
    import json as _json
    zones_json = _json.dumps(zones or [])
    try:
        with get_conn() as conn:
            with conn.cursor(cursor_factory=RealDictCursor) as cur:
                cur.execute("""
                    INSERT INTO patient_notes (patient_id, note_date, content, zones)
                    VALUES (%s, %s, %s, %s)
                    RETURNING id, patient_id, content, zones,
                              note_date::text AS note_date,
                              created_at::text AS created_at
                """, (patient_id, note_date, content, zones_json))
                row = cur.fetchone()
            conn.commit()
            return dict(row)
    except Exception as e:
...
def list_patient_notes(patient_id: int) -> list:
    ...
    try:
        with get_conn() as conn:
            with conn.cursor(cursor_factory=RealDictCursor) as cur:
                cur.execute("""
                    SELECT id, patient_id, content, zones,
                           note_date::text AS note_date,
                           created_at::text AS created_at
                    FROM patient_notes
                    WHERE patient_id = %s
...
def update_patient_note(note_id: int, content: str, zones: list = None) -> bool:
    if not DATABASE_URL:
        return False
    try:
        with get_conn() as conn:
            with conn.cursor() as cur:
                if zones is not None:
                    import json as _json
                    cur.execute("""
                        UPDATE patient_notes SET content = %s, zones = %s WHERE id = %s
                    """, (content, _json.dumps(zones), note_id))
                else:
                    cur.execute("""
