...
def email_exists(email: str) -> bool:
    """Return True if this email already has a submission in the database."""
    if not DATABASE_URL:
        return False
    try:
        with get_conn() as conn:
...
def submission_exists(email: str, year: int, month: int, day: int) -> bool:
    """Return True if this exact email + date of birth already has a record."""
    if not DATABASE_URL:
        return False
    try:
        with get_conn() as conn:
...
def update_notes(sub_id: int, notes: str) -> bool:
    ...
    try:
        ...
        return False    if not DATABASE_URL:
        logger.warning("DATABASE_URL not set — database features disabled.")
        return
    try:
        with get_conn() as conn:
